...
function ExperimentsPage() {
  ...
  const generateExperiments = async () => {
    ...
    try {
      ...
      const response = await runAnalysis(selectedPaper, "experiments");
      const rawResult =
      ...
      if (!rawResult) {
        throw new Error("No experiment protocol received from backend");
      }

      setProtocol(stripAllSymbols(rawResult));
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Experiment generation failed"
      );
    } finally {
      setLoading(false);
    }
  ...
  const copyProtocol = async () => {
    ...
    try {
      ...
      setError("Failed to copy experimental protocol");
    }
  };
  ...
  const downloadProtocol = () => {
    ...
    const content = [
      "ResearchX — Experimental Replication Protocol",
      "",
      `Paper: ${selectedPaper}`,
      "",
      protocol,
    ...
    anchor.href = url;
    anchor.download = `${safeFilename}-experiment-protocol.txt`;
  ...
  return (
    <div className="min-w-0">
      <PageHeader
        ...
        subtitle={
          selectedPaper
            ? `Experimental protocol and validation setup for '${selectedPaper}'.`
      ...
      <div className="rounded-2xl glass p-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-center">
          ...
          >
            ...
            {loading ? "Designing..." : "Design Experiments"}
          </Button>
        </div>
      </div>
      ...
      {loading && (
        <div className="mt-6 rounded-2xl glass p-10 text-center">
          ...
          <h3 className="mt-4 font-semibold">Designing Experimental Protocol</h3>
          <p className="mt-2 text-sm text-muted-foreground">
            Formulating validation hypotheses, baselines, and execution steps...
          </p>
        </div>
      )}
      ...
      {protocol && !loading && (
        <div className="mt-6 rounded-2xl glass p-7">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <h3 className="text-base font-semibold">Experimental Validation Plan</h3>
