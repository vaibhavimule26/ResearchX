...
export async function runAnalysis(
  ...
  if (!paperName?.trim() && !query?.trim()) {
    throw new Error("Either a research paper or query is required.");
  }
  ...
  if (paperName?.trim()) {
    payload.paper_name = paperName.trim();
  }
  if (query?.trim()) {
  ...
  console.log(
    "ResearchX Coordinator Analysis:",
    payload
  );
